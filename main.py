...
def scrape_question(url, driver, output_file):
    driver.get(url)
    time.sleep(16)  # Wait for the page to load (adjust as needed)
    ...
    try:
        ...
        if "Professional Data Engineer" in driver.page_source:
            ...
            # Save to file
            with open(output_file+'ML', 'a', encoding='utf-8') as file:
                file.write(f"Link: {url}\n")
                file.write(f"Question Number: {question_number}\n")
                file.write(f"Question: {question}\n")
                file.write("Possible Answers:\n")
...
def organize():
    ...
    output_file_path = "organized_questionsML.txt"
...
def main():
    ...
    start_number = 130358
    end_number = start_number + 500 # You can adjust the range as needed
